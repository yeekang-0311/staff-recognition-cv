...
def detectTag(torso_model, tag_model, oriPic, device, half):
  torso_coor = detectTorso(torso_model, oriPic, device, half)
#   no torso detected 
  if torso_coor == None:
    return False
  ...
  with torch.no_grad():
    ...
    pred = non_max_suppression(pred, 0.50, 0.45, [0])
  ...
  for i, det in enumerate(pred):  # detections per image
    if len(det):
        ...
        for *xyxy, conf, cls in reversed(det):
            ...
            print(f"Tag X {centerX} Y {centerY}, Torso: {torso_coor}")
            # if detection not within the torso bounding box return false
            if not (centerX > torso_coor[0] and centerX < torso_coor[2] and centerY > torso_coor[1] and centerY < torso_coor[3]):
                continue
...
def detect(save_img=False):
    ...
    # model for tag recognition
    tag_model = attempt_load("./tagmodel.pt", map_location=device) 
    # model for keypoint estimation (torso identification)
    weigths = torch.load('yolov7-w6-pose.pt', map_location=device)
    torso_model = weigths['model']
    _ = torso_model.float().eval()
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if isStaff:
                            label = f' Staff {conf:.2f}'
                            plot_one_box(xyxy, im0, label=label, color=55, line_thickness=1)
                        else:
